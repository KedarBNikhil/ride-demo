...
export function EndRideScreen({ rideId, onConfirmed, onIssueDone }: { rideId: string; onConfirmed: () => void; onIssueDone: () => void }) {
  ...
  const baseFare = Number(ride?.base_fare ?? ride?.estimated_fare ?? 0); const adjustment = Number(ride?.distance_surcharge ?? 0) + Number(ride?.pickup_surcharge ?? 0); const gross = Number(payout?.captain_earning_amount ?? ride?.final_fare ?? ride?.estimated_fare ?? 0); const deduction = Number(payout?.dispute_deduction ?? 0); const total = Number(payout?.net_earning_amount ?? gross);
