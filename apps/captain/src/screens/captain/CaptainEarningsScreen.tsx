  ...
  return <SafeAreaView style={styles.safe} edges={['top', 'left', 'right']}>
    <ScrollView contentContainerStyle={[styles.content, { paddingBottom: tabBarHeight + 24 }]} showsVerticalScrollIndicator={false}>
      ...
      {!loading && !failed && <>
        ...
        <View style={[styles.breakdown, shadows.card]}><BreakdownRow label={t('captain.rideEarnings')} value={formatFare(overview?.rideEarnings ?? 0)} />{(overview?.adjustments ?? 0) > 0 && <BreakdownRow label="Dispute deduction" value={`-${formatFare(overview?.adjustments ?? 0)}`} />}<BreakdownRow label={t('captain.tips')} value={overview?.tips == null ? '—' : formatFare(overview.tips)} /><BreakdownRow label={t('captain.bonuses')} value={overview?.bonuses == null ? '—' : formatFare(overview.bonuses)} /><View style={styles.divider} /><BreakdownRow label="Net earning" value={formatFare(overview?.totalEarnings ?? 0)} total /></View>
