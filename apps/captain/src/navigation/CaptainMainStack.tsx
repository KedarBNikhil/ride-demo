...
import { CaptainPaymentIssueSupportScreen } from '../screens/captain/CaptainPaymentIssueSupportScreen';
import { YourDocumentsScreen } from '../screens/captain/YourDocumentsScreen';
...
import { subscribeToDirectCaptainOfferResponses } from '../services/directCaptainRideOffers';
...
import { activeRideGpsTracking } from '../services/activeRideGpsTracking';
...
function CaptainRideCancellationGuard({ rideId, onCancelled, onReleased, children }: { rideId: string; onCancelled: (ride: DispatchRide) => void; onReleased: () => void; children: React.ReactNode }) {
  useEffect(() => rideDispatchService.subscribeToRide(rideId, (ride) => {
    if (ride.status === 'cancelled') onCancelled(ride);
    else if (ride.status === 'searching') onReleased();
  }), [onCancelled, onReleased, rideId]);
  return <>{children}</>;
}
...
function CaptainRideCancellationFlow({ rideId, onDone, children }: { rideId: string; onDone: () => void; children: React.ReactNode }) {
  ...
  const endTracking = useCallback(() => { void activeRideGpsTracking.stop({ flush: false }).catch(() => undefined); }, []);
  return <CaptainRideCancellationGuard rideId={rideId} onCancelled={(ride) => { endTracking(); setCancelledRide(ride); }} onReleased={() => { endTracking(); onDone(); }}><View style={{ flex: 1 }}>{children}{cancelledRide && <CaptainRideCancelledSheet ride={cancelledRide} onDone={onDone} />}</View></CaptainRideCancellationGuard>;
}
...
export function CaptainMainStack({ online, onToggle, onLanguageChange, onTripComplete, onAccountDeleted }: { online: boolean; onToggle: () => void; onLanguageChange: (language: AppLanguage) => Promise<void>; onTripComplete: () => void; onAccountDeleted: () => void }) {
  ...
  useEffect(() => { void rideDispatchService.getCaptainActiveRide().then((ride) => activeRideGpsTracking.reconcile(ride ? { id: ride.rideId, status: ride.status } : null)).catch(() => undefined); }, []);
  ...
  useEffect(() => subscribeToDirectCaptainOfferResponses(({ rideId, offerId }) => {
    void rideDispatchService.getCaptainPendingOffer(rideId, offerId).then((offer) => {
      if (offer) presentRequest(offer);
      else incomingRideAlertService.stop(offerId);
    }).catch(() => undefined);
  }), [presentRequest]);
  useEffect(() => {
  ...
  return <Stack.Navigator screenOptions={{ headerShown: false, animation: 'slide_from_right', animationDuration: 260 }}>
    ...
    <Stack.Screen name="CaptainRideDetails">{({ navigation, route }) => { const { rideId } = route.params as { rideId: string }; return <CaptainRideDetailsScreen rideId={rideId} onBack={() => navigation.goBack()} onOpenSupportChat={() => navigation.navigate('CaptainPaymentIssueSupport', { rideId })} />; }}</Stack.Screen>
    <Stack.Screen name="CaptainPaymentIssueSupport">{({ navigation, route }) => <CaptainPaymentIssueSupportScreen rideId={(route.params as { rideId: string }).rideId} onBack={() => navigation.goBack()} />}</Stack.Screen>
    ...
    <Stack.Screen name="EndRide" options={{ gestureEnabled: false }}>{({ navigation, route }) => { const params = route.params as { rideId: string }; return <EndRideScreen rideId={params.rideId} onConfirmed={() => navigation.replace('RateCustomer', { rideId: params.rideId })} onIssueDone={() => { onTripComplete(); navigation.reset({ index: 0, routes: [{ name: 'CaptainHome' }] }); }} />; }}</Stack.Screen>
    ...
    <Stack.Screen name="Settings">{({ navigation }) => <SettingsScreen profile ratingRole="captain" onBack={() => navigation.goBack()} onLanguageChange={(language) => { void onLanguageChange(language).then(() => navigation.goBack()); }} onAbout={() => navigation.navigate('About')} onDocuments={() => navigation.navigate('YourDocuments')} onAccountDeleted={onAccountDeleted} />}</Stack.Screen>
    <Stack.Screen name="YourDocuments">{({ navigation }) => <YourDocumentsScreen onBack={() => navigation.goBack()} />}</Stack.Screen>
