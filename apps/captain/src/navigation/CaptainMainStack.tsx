...
function CaptainRideCancellationFlow({ rideId, onDone, onSurchargeTimeout, children }: { rideId: string; onDone: () => void; onSurchargeTimeout?: () => void; children: React.ReactNode }) {
  ...
  return <CaptainRideCancellationGuard rideId={rideId} onCancelled={(ride) => { endTracking(); setCancelledRide(ride); }} onReleased={() => { endTracking(); onDone(); }} onSurchargeTimeout={onSurchargeTimeout}><View style={{ flex: 1 }}>{children}{cancelledRide && <CaptainRideCancelledSheet ride={cancelledRide} onDone={onDone} />}</View></CaptainRideCancellationGuard>;
}
