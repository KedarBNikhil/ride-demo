...
export function CustomerAppNavigator() {
  ...
  const clearCustomerRide = useCallback(() => { setCustomerRideStatus('cancelled'); setCustomerRide({ pickup: '', drop: '', kind: 'bike', passengerCount: 1 }); }, []);
  ...
  const openCustomerHome = useCallback(() => { void reconcileCurrentCustomerRide().then((currentRide) => { if (!currentRide && navigationRef.isReady() && currentRouteName() !== 'CustomerHome') navigationRef.reset({ index: 0, routes: [{ name: 'CustomerHome' as never }] }); }).catch(() => undefined); }, [reconcileCurrentCustomerRide]);
  ...
  useEffect(() => { if (!customerRide.id || customerRide.id.startsWith('local-')) return; return rideDispatchService.subscribeToRide(customerRide.id, (ride) => { if (ride.status === 'cancelled' && ride.fare_approval_status === 'declined' && ride.cancellation_reason_detail === 'Customer did not confirm updated captain-distance fare within 30 seconds') { clearCustomerRide(); if (navigationRef.isReady()) navigationRef.reset({ index: 0, routes: [{ name: 'RideType' as never }] }); return; } setCustomerRideStatus(ride.status); void reconcileCurrentCustomerRide().catch(() => undefined); }); }, [clearCustomerRide, customerRide.id, reconcileCurrentCustomerRide]);
