-- Pickup OTPs are always per-ride, backend-generated values. They must not
-- depend on the pilot-auth feature flag or accept a development fallback.
...
begin
  ...
  if not found or p_pickup_otp !~ '^[0-9]{6}$' or v_ride.pickup_otp_digest is null or v_ride.pickup_otp_expires_at <= now() or v_ride.pickup_otp_attempt_count >= 5 then
...
revoke all on function public.issue_customer_pickup_otp(uuid), public.captain_start_ride(uuid, text) from public, anon;
grant execute on function public.issue_customer_pickup_otp(uuid), public.captain_start_ride(uuid, text) to authenticated;
