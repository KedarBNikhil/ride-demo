-- A cancelled offer must not retain a response timestamp: the live constraint
-- permits responded_at only for accepted or rejected offers.
...
begin
  ...
  loop
    update public.ride_offers set status = 'cancelled', responded_at = null where ride_id = v_ride_id and status = 'accepted';
