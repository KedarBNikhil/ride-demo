...
export function LiveLocationMap({ mapRef, location, onMapReady, children }: { mapRef: React.RefObject<MapView | null>; location: LiveCoordinate | null; onMapReady?: () => void; children?: React.ReactNode }) {
  if (!hasGoogleMapsKey) return <View style={styles.fallback}><View style={styles.dot} /><Text style={styles.fallbackText}>Location is active. Map preview is unavailable in this build.</Text></View>;
