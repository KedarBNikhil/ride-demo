...
const config: ExpoConfig = {
  ...
  android: {
    ...
    permissions: isCaptain ? ['ACCESS_COARSE_LOCATION', 'ACCESS_FINE_LOCATION', 'CAMERA'] : ['ACCESS_COARSE_LOCATION', 'ACCESS_FINE_LOCATION'],
  },
  ios: {
  ...
  plugins: isCaptain ? [['expo-image-picker', { cameraPermission: 'Allow Nandyal Ride Captain to use your camera for documents.', photosPermission: 'Allow Nandyal Ride Captain to access document photos.' }]] : [],
  extra: { googlePlacesApiKey: process.env.EXPO_PUBLIC_GOOGLE_PLACES_API_KEY ?? '', appMode: variant },
};
